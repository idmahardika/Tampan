				...
				foreach($theusers as $auser)
				{
					...
					?>
						<tr <?php if($count++ % 2 == 0) { ?>class="alternate"<?php } ?>>
							...
							<td>
								<?php echo $auser->ola; ?>
								<form method="post" <?php echo 'action="' . plugins_url( '/ola.php', __FILE__ ) . '">';?>
									<select name="ola">
										<option value="inactive">inactive</option>
										<option value="active" select="selected">active</option>
									</select>
									<input type="text" name="uid" value="<?php echo $theuser->ID; ?>" style="display:none;" />
